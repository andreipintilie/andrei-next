...
const Header = ({ className = '' }) => {
  ...
  return (
    <header
      className={`bg-primary text-white w-full fixed z-1 left-1/2 -translate-x-1/2 h-(--header-height) top-0 ${className}`}
    >
      <div className='flex justify-between items-center gap-4 mx-auto py-6 containerSecondary'>
        ...
        <div className='group lg:hidden borderColor'>
          <button
            className='lg:hidden justify-center items-center grid bg-black-primary group-hover:bg-linear-to-r group-hover:from-secondary group-hover:to-tertiary border border-none rounded-primary w-10 h-10 font-bold text-white group-hover:text-black text-small transition-colors duration-300 cursor-pointer'
            type='button'
            onClick={toggleMenu}
          >
